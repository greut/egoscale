...
func NewClient(cfs ...ConfigFunc) (*Client, error) {
	...
	computeClient, err := compute.NewClient(context.Background(),
		profile.APIKey,
		profile.APISecret,
		profile.ComputeAPIEndpoint,
		tracing,
	)
	if err != nil {
	...
	dnsClient, err := dns.NewClient(context.Background(),
		profile.APIKey,
		profile.APISecret,
		profile.DNSAPIEndpoint,
		tracing,
	)
	if err != nil {
	...
	runstatusClient, err := runstatus.NewClient(context.Background(),
		profile.APIKey,
		profile.APISecret,
		profile.RunstatusAPIEndpoint,
		tracing,
	)
	if err != nil {
